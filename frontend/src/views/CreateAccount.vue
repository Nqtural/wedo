...
const variant_password_repeat = ref<InputVariant>("normal");
...
async function submit() {
	...
	if (password.value !== password_repeat.value) {
		...
		variant_password_repeat.value = "danger";
		return;
	}
	...
	try {
		...
		if (error instanceof Error && error.message === "HTTP error: 409") {
...
<template>
	<div class="wrapper">
		...
		<form @submit.prevent="submit()">
			...
			<Input
				type="password"
				@input="variant_password_repeat = 'normal'"
				:variant="variant_password_repeat"
			...
			<p>
				...
				<RouterLink to="/login">Log in</RouterLink> instead!
			</p>
		</form>
	</div>
</template>
